...
export const sansSerif = ({ size = 18 } = {}) => {
  const ss = parser.parseFromString(_sansSerif(), 'image/svg+xml').childNodes[0]
  ss.style.width = size + 'px';
  return ss
}

export const serif = ({ size = 18, adjusted } = {}) => {
  const s = parser.parseFromString(_serif(), 'image/svg+xml').childNodes[0]
...
export const warning = ({ size = 150 } = {}) => {
  const ss = parser.parseFromString(_warning(), 'image/svg+xml').childNodes[0]
  ss.style.width = size + 'px';
  return ss
}
